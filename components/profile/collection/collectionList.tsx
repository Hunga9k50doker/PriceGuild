...
const CollectionList = ({
  ...
  const onCreateSuccess = async (e: any) => {
    ...
    if (!e.isEdit) {
      ...
      setData(
        paginate(
          collections?.data?.map((item) => (item.group_ref === collectionDetail?.group_ref ? { ...item, group_name: e.name, type: e.type } : item)),
          rowsPerPage,
          pagesSelected
        )
      );
  ...
  const gotoCard = (item: ManageCollectionType) => {
    props.gotoCard
      ? props.gotoCard(item)
      : router.push(
          `/profile/${!isEmpty(router.query.page) && Boolean(Number(router.query.page)) ? router.query.page + "/" : ""}${
            title === "collection" ? t("portfolio.text_normal") : title + "s"
          }/${item.group_ref}/${encodeURIComponent(item.group_name.replaceAll("/", "-"))}`
        );
  };
  ...
  const onClaimPhoto = async (id?: number) => {
    try {
      ...
      // @ts-ignore
      document.getElementById(`dropdownMenuButton${id}`).className = "btn btn-secondary dropdown-toggle";
      // @ts-ignore
  ...
  const onLoadMore = () => {
    if (pagesSelected[pagesSelected.length - 1] + 1 <= Math.ceil((collections.data.length ?? 0) / rowsPerPage)) {
      const pageNew = [...pagesSelected, pagesSelected[pagesSelected.length - 1] + 1];
  ...
  return (
    <>
      ...
      {!isEmpty(router.query.page) && Boolean(Number(router.query.page)) && (
        <HeaderUser userId={Number(router.query.page)} onTabDetail={onTabDetail} sendMessage={() => {}} isFriend={true} friend={profileFriend} />
      )}
      <div className="profile-personal-collections">
        <div className="mt-4 profile-personal-collections-head">
          ...
          {Boolean(dataSearch.length > 0 || searchKey !== "") && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              ...
              {isButtonRight && (
                <div className="d-flex btn-group-filter">
                  ...
                  <button type="button" onClick={() => onHandleModal(true)} className="btn text-capitalize btn-collection-primary">
                    {" "}
                    Create {`${title === "collection" ? t("portfolio.text") : title}`}{" "}
                  </button>
                </div>
              )}
            </div>
        ...
        <div className="row profile-personal-collections-content">
          ...
          {data.map((item, index) => (
            <div key={index} onClick={() => gotoCard(item)} className="col-12 col-md-6 col-lg-6 col-sm-6 mb-4">
              <div className="card p-3 cursor-pointer">
                <div className="d-flex justify-content-between align-items-center">
                  <div title={item.group_name} className="col-10 text-truncate title fw-bold">
                    {item.group_name} {Boolean(item.type === 2) && <i className="ms-1 ic-padlock" aria-hidden="true"></i>}
                    {Boolean(item.type === 0) && item.group_ref === 0 && <img src={IconFolder} alt="icon-folder" />}{" "}
                  </div>
                  {isEdit && (
                        ...
                        <ul className="dropdown-menu" id={`dropdownMenu${item.group_ref}`} aria-labelledby={`dropdownMenuButton${item.group_ref}`}>
                          ...
                          {item.group_ref !== 0 && (
                            <li
                              onClick={(e) => {
                                e.stopPropagation();
                                onConfirmRemove(item?.group_ref ?? 0);
                                setDataSelect(item?.group_name ?? "");
                              }}
                            >
                              <span className="dropdown-item text-capitalize">
                                {" "}
                                Remove {`${title === "collection" ? t("portfolio.text") : title}`}{" "}
                              </span>
                            </li>
                          )}
                        </ul>
          ...
          {!collections.isLoading && Boolean(collections.data.length) && (
            <>
              {Boolean(pagesSelected[pagesSelected.length - 1] < Math.ceil((collections.data.length ?? 0) / rowsPerPage)) && (
              ...
              <div className="d-flex justify-content-center mt-3 pagination-page">
                <Pagination
                  ...
                  totalPage={Math.ceil((collections.data.length ?? 0) / rowsPerPage)}
                />
              </div>
            </>
          )}
        </div>
        {collections.data.length === 0 && !collections.isLoading && searchKey === "" && Boolean(!matchPatchRoute) && (
          <div className="empty-collection">
            <div className="box-content">
              ...
              <button className="btn btn-primary" onClick={() => onHandleModal(true)}>
                Create {title === "wishlist" ? "Wishlist" : t("portfolio.text")}
              </button>
            </div>
          </div>
        )}
        {matchPatchRoute && !collections.isLoading && collections.data.length === 0 && searchKey === "" ? (
          <div className="message-profile-null">
            {profileFriend && profileFriend?.user_info?.full_name
              ? profileFriend?.user_info?.full_name
              : profileFriend?.user_info?.username
              ? `@${profileFriend?.user_info?.username}`
              : ""}{" "}
        ...
        {collections && collections.data.length === 0 && !collections.isLoading && searchKey !== "" && (
          <div className="message-profile-null">No results were found matching keyword</div>
        )}
        <Collection
          onConfirmRemove={() => {
            setIsOpenModal(true);
        ...
        <ModalDeletePortfolio
          ...
          } data is permanent and cannot be undone. All cards that are grouped in <span class="text-color-portfolio">${dataSelect}</span> ${
            title === "wishlist" ? "wishlist" : t("portfolio.text_normal")
          } will be deleted.`}
