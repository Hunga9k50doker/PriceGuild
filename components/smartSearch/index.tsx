...
function SmartSearch(props: PropsType) {
      ...
      try {
        ...
        if (res.success) {
          ...
          if (props.sportId || props.sportName) {
            const sportSelected = res.data.find(item => item.id === props.sportId || item?.sportName.replace(/\s/g, '')?.toLowerCase() === props.sportName) ?? defaultSport;
  ...
  return (
    <div className={`d-flex smart-search-card ${props.isHomePage && props.isHomePage == 1 ? 'smart-search-card-home' : ''} ${props.isArrow == 1 ? 'show' : ''}`}>
