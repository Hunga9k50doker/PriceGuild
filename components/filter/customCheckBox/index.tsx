...
const CheckBoxFilter = React.forwardRef<FilterHandle, PropsType>((props, ref) => {
  ...
  const renderLength = () => {
    return sumBy(optionsSearch ?? options, function (o) {
      return o.options?.length ?? 1;
    });
  };

  const checker = (arr: any, target: any) =>
    target.every((v: any) => checkedState?.map((item) => item.id).includes(v.id));

  const renderContent = (e: FilterType, i: number) => {
    if (e?.options) {
      return (
        <div key={i} className={`checkbox-select-list ${props.isMultipleLv ? "multiple-Lv" : ""} `}>
          {props.isMultipleLv ? (
          ...
          <div className="checkbox-select__search-wrapp">
            {props.isSearch !== false && (
              <div className="d-flex checkbox-select__search-wrapp-input">
                ...
                {Boolean(keySearch) && (
                  <i onClick={onClearSearch} className="ic-close-input input-close" aria-hidden="true"></i>
                )}
              </div>
            )}
          </div>
