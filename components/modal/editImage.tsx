...
const DynamicComponentWithNoSSR = dynamic<EditorProps>(() => import("./EditImageNoSSR/index"), { ssr: false });
    ...
    <Modal onHide={onClose} centered show={isOpen} size="lg" className="modal-profile-collection">
      <div>
        ...
        <Modal.Footer className="modal-footer-crop">
          <button className="btn btn-secondary btn-bg--secondary btn-color--primary" onClick={onClose}>
            Cancel
          </button>
          <button disabled={isLoading} onClick={saveImageToDisk} className="btn btn-secondary btn-bg--primary btn-color--white">
            Done {isLoading && <span className="spinner-grow spinner-grow-sm" role="status" aria-hidden="true" />}
          </button>
        </Modal.Footer>
      </div>
    </Modal>
