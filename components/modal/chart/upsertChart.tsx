...
const UpsertChart = ({ isOpen, setIsOpen, collection, chartDetail, onSuccess, onConfirmRemove }: PropTypes) => {
  ...
  return (
    <Modal
      ...
      show={isOpen}
      fullscreen="sm-down"
      className="modal-profile-collection modal-content--radius modal-profile-collection-chart add-collection-right profile-chart"
    >
      <Modal.Header>
        <Modal.Title>{isEmpty(chartDetail) ? "Add" : "Edit"} Widget</Modal.Title>
        <button onClick={() => setIsOpen(false)} type="button" className="close" data-dismiss="modal" aria-label="Close">
      ...
      <Modal.Body className="customScroll p-32">
        <form>
          <div className="row col-mar-10">
                ...
                <div className="d-flex w-100">
                  ...
                  <div className="btn-group-item w-25">
                    ...
                    <label className="btn btn-outline-primary w-100" htmlFor="chartBar">
                      ...
                      <span>
                        Bar <span className="only-desktop">Chart</span>
                      </span>
                    </label>
                  </div>
                  <div className="btn-group-item w-25">
            ...
            <div className="mb-3 custom-select-56">
              <label htmlFor="" className="form-label">
                Use purchase price for missing data
              </label>
              <div className="">
                <Controller
                  control={control}
            ...
            <div>
              {!isEmpty(chartDetail) && (
                <div className="d-flex justify-content-center mt-3 btn-group-remove">
                  <a href="" onClick={onDelete} className="d-flex justify-content-center align-items-center btn-remove mb-0">
                    <img src={IconDelete} alt="Remove Widget" /> <span>Remove Widget</span>
                  </a>
                </div>
              )}
            </div>
      ...
      <Modal.Footer className="custom-modal-footer">
        <button className="btn btn-outline--custom color-124DE3 h-56" onClick={() => setIsOpen(false)}>
          Cancel
        </button>
        <button disabled={isLoading} onClick={handleSubmit(onSubmit)} className="btn h-56 btn-collection-primary p-0 ml-3">
          {isEmpty(chartDetail) ? "Add" : "Update"} Widget
