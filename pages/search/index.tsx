...
const CardList = (props: PropTypes) => {
  ...
  const getFilterSearch = () => {
    {
      console.log(filterData, "1844444444444");
    }
    let params: any = {};
    let filterOld = {};
    if (Boolean(isFilterStore)) {
    ...
    console.log(filterOld, "2211111111111111111");
    for (const [key, value] of Object.entries(filterOld ?? {})) {
      ...
      if (key === "printRun") {
        // @ts-ignore
        if (value.length) {
          params.printRun = [[arrayValue.includes(1), arrayValue.includes(2)]];

          if (arrayValue.includes(3)) {
            params.printRun.push([null, 1]);
          }
          if (arrayValue.includes(4)) {
            params.printRun.push([2, 24]);
          }
          if (arrayValue.includes(5)) {
            params.printRun.push([25, 99]);
          }
          if (arrayValue.includes(6)) {
            params.printRun.push([100, 299]);
          }
          if (arrayValue.includes(7)) {
            params.printRun.push([299, null]);
          }
        }
      } else if (key === "sport" && arrayValue[0] === "all") {
  ...
  const getListCard = async (
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result?.error) ToastSystem.error(result?.error);
      }
    ...
    if (!prioritize.find((item) => item.name === key)) {
      ...
      if (prioritize[prioritize.length - 1].name === key && e.length === 0) {
        ...
        dataSave = dataSave.map((item) =>
          item.name === key ? { ...item, isChange: true } : { ...item, isChange: false }
        );
        setPrioritize(dataSave);
      }
    }
    ...
    if (filterData?.type?.length) {
      ...
      options = chain(options)
        ...
        .map((value, key) => ({
          id: key,
          value: value[0].id,
          name: `${value.length > 2 ? value[0].name + " - Various Print Runs" : value[0].name}`,
        }))
        .value();
    }
    return options;
  ...
  const selectCollection = (item: ManageCollectionType) => {
    ...
    getDataOptionInput();
    router.push(`/collections-add-card?collection=${item.group_ref}&code=${cardPortfolio.toString()}`);
  };
  ...
  const renderSportName = () => {
    ...
    return (
      query.q &&
      ((filterData?.sport?.[0]?.name && filterData?.sport?.[0]?.name !== "All") || sportName) && (
        <>
          in{" "}
          <span
            onClick={() => router.push(`/search?sport_criteria=${filterData?.sport?.[0]?.id ?? sportName?.id}`)}
            className="sport-name cursor-pointer"
          >
            “{filterData?.sport?.[0]?.name ?? sportName?.sportName}”
          </span>
        </>
      )
    );
  ...
  React.useEffect(() => {
    if (isChangeRouter && filters.years.length && filters.publishers.length) {
      ...
      if (sportState) {
        ...
        // @ts-ignore
        prioritizeState = prioritizeState.map((item) => ({
          ...item,
          isChange: false,
        }));
    ...
    return (
      <>
        {(Boolean(!checkFilter(filterOld ?? {})) ||
          !isEmpty(cardNumberRef.current?.getValue()) ||
          !isEmpty(playerNameRef.current?.getValue())) && (
          <button onClick={() => resetPage(false)} className="btn btn-primary clear-select">
            {" "}
            Reset Filters{" "}
          </button>
        )}
    ...
    switch (filterValue) {
      ...
      case "sport":
        return Boolean(query.q)
          ? ""
          : sumBy(filters.sports, function (o) {
    ...
    switch (filterValue) {
      case "publisher":
        return Boolean(filterData?.publisher?.length) ? (
          ...
          >
            {" "}
            Clear Selected ({filterData?.publisher?.length}){" "}
          </button>
        ) : (
          ""
        );
      case "year":
        return Boolean(filterData?.year?.length) ? (
          <button
            ...
            className="btn btn-primary clear-select"
          >
            Clear Selected ({filterData?.year?.length}){" "}
          </button>
        ) : (
          ""
        );
      case "set":
        return Boolean(filterData?.set?.length) ? (
          ...
          >
            {" "}
            Clear Selected ({filterData?.set?.length}){" "}
          </button>
        ) : (
          ""
        );
      ...
      case "type":
        return Boolean(filterData?.type?.length) ? (
          ...
          >
            {" "}
            Clear Selected ({filterData?.type?.length}){" "}
          </button>
        ) : (
          ""
        );
      case "color":
        return Boolean(filterData?.color?.length) ? (
          ...
          >
            {" "}
            Clear Selected ({filterData?.color?.length}){" "}
          </button>
        ) : (
          ""
        );
      case "printRun":
        return Boolean(filterData?.printRun?.length) ? (
          ...
          >
            {" "}
            Clear Selected ({filterData?.printRun?.length}){" "}
          </button>
        ) : (
          ""
        );
    ...
    return (
      <>
        {(Boolean(!checkFilter(filterOld ?? {})) ||
          !isEmpty(cardNumberRef.current?.getValue()) ||
          !isEmpty(playerNameRef.current?.getValue())) && (
          <div onClick={() => resetPage(false)} className="btn btn-primary clear-select">
            <div> Reset Filters </div>
          </div>
        )}
      </>
  ...
  const renderNumberFilter = () => {
    ...
    return (
      <>
        {Boolean(!checkFilter(filterOld ?? {})) && (
          <span className="filter-number">{Object.values(filterOld).flat().length}</span>
        )}
      </>
    );
  };
  ...
  const onSortTable = (name: string) => {
    if (data?.rows) {
      let itemSort = MetaData.sort_card_list.find(
        (item: any) => item?.sort_value === name && item?.sort_by === sortCards.sort_by
      );
  ...
  const renderCompareIcon = (data: any) => {
    return Boolean(cards.find((item) => item.code === data.code)) ? IconCanFull : IconCan;
  };
        ...
        <div className="col-lg-10 col-md-10 pt-5 pb-5 col-search-page">
          {
            ...
            width < 768 && (
              <>
                <div
                  className={`filter-mobile position-relative filter-mobile--search ${
                    !data.cards.length && !data.isLoading ? "d-none" : ""
                  }`}
                >
                  <div className="button-filter ">
                    ...
                    <button
                      onClick={() => setFilterValue("publisher")}
                      type="button"
                      className={`btn btn-primary btn-sm ${Boolean(filterData?.publisher?.length) ? "active" : ""}`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                      Publisher {Boolean(filterData?.publisher?.length) && <span>{filterData?.publisher?.length}</span>}{" "}
                    </button>
                    <button
                      onClick={() => setFilterValue("year")}
                      type="button"
                      className={`btn btn-primary btn-sm ${Boolean(filterData?.year?.length) ? "active" : ""}`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                      Year {Boolean(filterData?.year?.length) && <span>{filterData?.year?.length}</span>}
                    </button>
                    <button
                      onClick={() => setFilterValue("set")}
                      type="button"
                      className={`btn btn-primary btn-sm ${Boolean(filterData?.set?.length) ? "active" : ""}`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                      Collection {Boolean(filterData?.set?.length) && <span>{filterData?.set?.length}</span>}
                    </button>
                    <button
                      onClick={() => setFilterValue("auto_memo")}
                      type="button"
                      className={`btn btn-primary btn-sm ${Boolean(filterData?.auto_memo?.length) ? "active" : ""}`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                      {" "}
                      Autograph/Memorabilia{" "}
                      {Boolean(filterData?.auto_memo?.length) && <span>{filterData?.auto_memo?.length}</span>}
                    </button>
                    {Boolean(filterData?.set?.length) && (
                      <>
                        <button
                          ...
                          type="button"
                          className={`btn btn-primary btn-sm ${Boolean(filterData?.type?.length) ? "active" : ""}`}
                          data-bs-toggle="modal"
                          data-bs-target="#filterModal"
                        >
                          {" "}
                          Base/Insert {Boolean(filterData?.type?.length) && <span>{filterData?.type?.length}</span>}
                        </button>
                        {Boolean(filterData?.type?.length) && (
                          <button
                            onClick={() => setFilterValue("color")}
                            type="button"
                            className={`btn btn-primary btn-sm ${Boolean(filterData?.color?.length) ? "active" : ""}`}
                            data-bs-toggle="modal"
                            data-bs-target="#filterModal"
                          >
                            Parallel {Boolean(filterData?.color?.length) && <span>{filterData?.color?.length}</span>}
                          </button>
                        )}
                      </>
                    )}
                    <button
                      onClick={() => setFilterValue("printRun")}
                      type="button"
                      className={`btn btn-primary btn-sm ${Boolean(filterData?.printRun?.length) ? "active" : ""}`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                      {" "}
                      Print Run {Boolean(filterData?.printRun?.length) && <span>{filterData?.printRun?.length}</span>}
                    </button>
                    <button
                      onClick={() => setFilterValue("playerName")}
                      type="button"
                      className={`btn btn-primary btn-sm ${
                        playerNameRef.current && playerNameRef.current.getValue() !== "" ? "active" : ""
                      }`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                    ...
                    <button
                      onClick={() => setFilterValue("cardNumber")}
                      type="button"
                      className={`btn btn-primary btn-sm ${
                        cardNumberRef.current && cardNumberRef.current.getValue() !== "" ? "active" : ""
                      }`}
                      data-bs-toggle="modal"
                      data-bs-target="#filterModal"
                    >
                                  ...
                                  <div className="sidebar__categories">
                                    <div className="section-title">
                                      <SortMobile
                                        ...
                                        value={sortCards}
                                        options={MetaData.sort_card_list}
                                      />
                                    </div>
                                  </div>
                                </div>
                    ...
                    >
                      <div
                        className={`modal-dialog ${
                          filterValue === "all" ? "modal-all" : "align-items-end"
                        }  modal-filter modal-lg modal-dialog-centered ${filterValue === "sport" ? "modal-sport" : ""}
                          ...
                          <div className="modal-header">
                            ...
                            <h5 className="modal-title" id="filterModalLabel">
                              ...
                              <span>
                                {filterValue === "sport" ||
                                filterValue === "all" ||
                                filterValue === "playerName" ||
                                filterValue === "cardNumber"
                                ...
                                <div className="shop__sidebar mt-3">
                                  <div className="sidebar__categories">
                                    <div className="section-title">
                                      <div
                                        className={`accordion ${
                                          filterValue === "sport" || filterValue === "all" ? "" : "d-none"
                                        }`}
                                        id="sportFilter"
                                      >
                                        <div className="accordion-item">
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "publisher" || filterValue === "all" ? "" : "d-none"
                                        }`}
                                        id="publisherFilter"
                                      >
                                        <div className="accordion-item">
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "year" || filterValue === "all" ? "" : "d-none"
                                        }`}
                                        id="YearFilter"
                                      >
                                        <div className="accordion-item">
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "set" || filterValue === "all" ? "" : "d-none"
                                        }`}
                                        id="CollectionFilter"
                                      >
                                        <div className="accordion-item">
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "auto_memo" || filterValue === "all" ? "" : "d-none"
                                        }`}
                                        id="auto_memoFilter"
                                      >
                                        <div className="accordion-item">
                                      ...
                                      {Boolean(filterData?.set?.length) && (
                                        <>
                                          <div
                                            className={`accordion ${
                                              filterValue === "type" || filterValue === "all" ? "" : "d-none"
                                            }`}
                                            id="typeFilter"
                                          >
                                            <div className="accordion-item">
                                          ...
                                          {Boolean(filterData?.type?.length) && (
                                            <div
                                              className={`accordion ${
                                                filterValue === "color" || filterValue === "all" ? "" : "d-none"
                                              }`}
                                              id="ParallelFilter"
                                            >
                                              <div className="accordion-item">
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "printRun" || filterValue === "all" ? "" : "d-none"
                                        }`}
                                        id="PrintRunFilter"
                                      >
                                        <div className="accordion-item">
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "playerName" || filterValue === "all" ? "" : "d-none"
                                        } 
                                    ${
                                      //@ts-ignore
                                      ...
                                      <div
                                        className={`accordion ${
                                          filterValue === "cardNumber" || filterValue === "all" ? "" : "d-none"
                                        } mb-3`}
                                        id="CardNumberFilter"
                                      >
                                        <TextSearchBoxDesktop
          ...
          <div
            className={`d-flex justify-content-between align-items-start p-head-search p-head-search--mobile ${
              isSelect ? "p-sticky-header p-sticky-header--full" : ""
            }`}
          >
            {isSelect ? (
              <div
                className={`d-flex align-items-center ml-1 btn-group-head-search ${scrollY > 400 ? "sticky-zero" : ""}`}
              >
              ...
              <div className="d-flex align-items-start p-head-search-total">
                <div className="me-2 mt-2 total-item-card">
                  {renderTotal()} found {renderSportName()}{" "}
                </div>
                <div className="d-flex flex-wrap p-head-search-filter--mobile">{resetFilterUI()}</div>
              </div>
            )}
            {isSelect && (
            ...
            <div className="action-list action-list--desktop d-flex justify-content-start align-items-center">
              ...
              <button
                ...
                className={`ms-2 ${
                  ...
                  isSelect ? "active" : ""
                } btn-search-plus d-flex justify-content-center align-items-center xxxxx`}
              ...
              <div className="action-list d-flex justify-content-start align-items-center">
                <div className="d-flex btn-group-card">
                  <button
                    ...
                    className={` ${!isInline ? "active" : ""} ms-2 btn btn-outline-secondary p-0`}
                  >
                ...
                <button
                  ...
                  className={`ms-2 ${
                    ...
                    isSelect ? "active" : ""
                  } btn-search-plus d-flex justify-content-center align-items-center xxxxxx`}
                  disabled={isInline && !cardSelected.length}
                >
                  {isSelect ? <IconMinis /> : <IconPlus />}
                </button>
              ...
              cardElement={(item: CardModel) => {
                return (
                  <CardElement
                    ...
                    // @ts-ignore
                    imageUrl={
                      item?.imgArr?.length && item?.imgArr[0] !== null
                        ? `https://img.priceguide.cards/${item.sport === "Non-Sport" ? "ns" : "sp"}/${
                            item?.imgArr[0]
                          }.jpg`
                        : undefined
                    }
            ...
            <>
              <div className="card-detail card-top-100 no-padding clear-margin-mobile">
                <div className="pricing-grid search-table-mode mt-3">
                  <div
                    className="content-pricing-grid content-pricing-grid-custom p-0 mt-2 mh-100 customScroll"
                    id="customScroll"
                    onScroll={onScroll}
                  >
                    <table className="table table-striped table-hover">
                      <thead className="p-sticky-header thead-search-table">
                        <tr>
                          ...
                          <th style={{ width: "15%" }} scope="col">
                            <div className="d-flex cursor-pointer align-items-center"> Min</div>
                          </th>
                          <th style={{ width: "15%" }} scope="col">
                            <div
                          ...
                          <th style={{ width: "15%" }} scope="col">
                            ...
                            >
                              ...
                              Average
                              <div className="ms-1 sort-table d-flex flex-column-reverse">
                                <i className={`sort-asc ${renderSortTable("price", true)}`} aria-hidden="true"></i>
                                <i className={`sort-desc ${renderSortTable("price", false)}`} aria-hidden="true"></i>
                              </div>
                            </div>
                          </th>
                          ...
                          <tr key={index}>
                              ...
                              <Link href={onGoToCard(item)}>
                                <a className="d-flex text-decoration-none c-dark-title-card">
                                  ...
                                  <div className="ps-3 collection-card-table-detail">
                                    ...
                                    <div className="mb-1  collection-card-desc fw-500 cursor-pointer">
                                      {" "}
                                      {`${item.webName} ${
                                        isEmpty(item?.onCardCode) ? "" : " - #" + item?.onCardCode
                                      }`}{" "}
                                    </div>
                            ...
                            <td>
                              ...
                              {item.minPrice !== null ? (
                                ...
                                <OverlayTrigger overlay={<Tooltip>{data.null_price_tooltip ?? ""}</Tooltip>}>
                                  {({ ref, ...triggerHandler }) => (
                            ...
                            <td>
                              {" "}
                              {
                                ...
                                item.avgPrice !== null ? (
                                  item.avgPrice ? (
                                    formatCurrency(item.avgPrice, currency)
                                  ) : (
                                    "N/A"
                                  )
                                ) : (
                            ...
                            <td>
                              <div className="dropdown dropdown--top">
                                <a
                                  ...
                                  aria-expanded="true"
                                  onClick={() => setOnActiveOption(false)}
                                >
                                  {" "}
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <div className="dropdown-menu-item__txt">
                                      {" "}
                                      {onActiveOption || Boolean(item.portfolio) ? "Added" : "Add"} to{" "}
                                      {t("portfolio.text")}{" "}
                                    </div>
                                  </div>
                                  {onActiveOption ? (
                                    <>
                                      <div
                                        onClick={(e) => {
                                          e.preventDefault();
                                          router.push(`/profile/portfolio/edit-card?collection=0&code=${item.code}`);
                                        }}
                                        className="dropdown-menu-item d-flex cursor-pointer"
                                      >
                                        <div className="dropdown-menu-item__icon mw-25">
                                          <img alt="" src={EditIconBlack} />
                                        </div>
